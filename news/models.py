...
class Author(models.Model):
    ...
    def update_rating(self):
        ...
        comments_rate = Comment.objects.filter(user_comment_id=self.user).aggregate(sum_articles=Coalesce(Sum('rating_comment'), 0))['sum_articles']
        comments_articles_rate = Comment.objects.filter(post_comment__author__users=self.user).aggregate(sum_posts=Coalesce(Sum('rating_comment'), 0))['sum_posts']
...
class Post(models.Model):
    ...
    type = models.CharField(max_length=2, choices=TYPE, default=news_)
